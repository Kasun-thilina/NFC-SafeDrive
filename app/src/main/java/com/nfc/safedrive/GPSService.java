...
public class GPSService extends Service implements LocationListener, GpsStatus.Listener {
    ...
    public void updateNotification(boolean asData){
        Notification.Builder builder = new Notification.Builder(getBaseContext())
                .setContentTitle("Running GPS Service")
                .setSmallIcon(R.drawable.ic_settings_black_24dp)
                .setContentIntent(contentIntent);

        if(asData){
            builder.setContentText(String.format("Speeed", data.getMaxSpeed(), data.getDistance()));
        }else{
            builder.setContentText(String.format("NA", '-', '-'));
        }
